	...
	function openAddDlg_dictionary() {
		parent.$.modalDialog({
			...
			onLoad:function() {
				...
				if(row) {
					var editForm = parent.$.modalDialog.handler.find("#form_id");
					editForm.find('input[name="categoryCode"]').val(row.id);
				}
			},
			buttons: [{
				text: '确定',
				...
				handler: function() {
					...
					parent.$.modalDialog.openner = $dg_category;//定义对话框关闭要刷新的组件
					var editForm = parent.$.modalDialog.handler.find("#form_id");
					editForm.attr("action", "uupm/dictionary/add");
					editForm.submit();
				}
			},{
				text: '取消',
	...
	function openEditDlg_dictionary() {
		...
		if(row) {
			parent.$.modalDialog({
				...
				href: 'views/uupm/dictionary/dictionaryEditDlg.jsp',
				onLoad:function(){
					$.post("uupm/dictionary/findOne", {categoryCode:row.id}, function(result){
						result = JSON.parse(result);
						if("OK"==result.status) {
				    		var editForm = parent.$.modalDialog.handler.find("#form_id");
							editForm.form("load", result.data);
		...
		if(row) {
			parent.$.messager.confirm("提示","确定要删除记录吗?",function(r){  
			    if(r) {
			    	var arr_id = getTreeIds(nodeDataWithChildren);
			    	var ids = arr_id.join(",");
			    	$.post("uupm/dictionary/del", {categoryCodes:ids}, function(result) {
						if(result.status=='OK') {
							$dg_category.tree('remove', row.target);
						}
						$.messager.show({
