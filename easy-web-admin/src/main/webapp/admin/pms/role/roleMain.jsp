...
// 			height: 28,
	...
	function openAddDlg() {
		parent.$.modalDialog({
			...
			width: 600,
			height: 400,
			href: 'admin/pms/role/roleEditDlg.jsp',
			buttons: [{
				text: '确定',
				iconCls: 'icon-ok',
				handler: function() {
					...
					editForm.attr("action", "pms/role/add");
					editForm.submit();
				}
			},{
				text: '取消',
			...
			parent.$.modalDialog({
				...
				buttons: [{
					...
					handler: function() {
						...
						parent.$.modalDialog.openner = $dg_role;//定义对话框关闭要刷新的grid
						var editForm = parent.$.modalDialog.handler.find("#form_id");
						editForm.attr("action", "pms/role/edit");
						editForm.submit();
					}
				},{
					text: '取消',
	...
	function removeFunc() {
		...
		if(row) {
			parent.$.messager.confirm("提示","确定要删除记录吗?",function(r){  
			    if(r) {
			    	$.post("pms/role/del", {roleCode:row.roleCode}, function(result) {
	...
	function findRoleFunc(rowIndex, rowData) {
		$.post("pms/role/findRoleResource", {roleCode:rowData.roleCode}, function(result) {
			if(result.statusCode=='OK') {
					...
					<table cellpadding="0" cellspacing="0">
						<tr>
							<td colspan="2" style="padding-left:2px;padding-bottom:2px;">
								<shiro:hasPermission name="role-set">
