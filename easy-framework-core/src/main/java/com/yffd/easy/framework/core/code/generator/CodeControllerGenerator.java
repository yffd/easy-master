 ...
 * @see 	 
 */
public class CodeControllerGenerator extends CodeGenerator {
	private static final String FILE_SUFFIX = ".java";
	private static final String TEMPLATE_PATH = "com\\yffd\\easy\\framework\\support\\code\\template\\XxxController.template";
	
	public void writeAllToFile(String modelRootDirPath, String modelPackageName, Class<?> superControllerClazz, String controllerPackageName, Class<?> serviceClazz, String author, String outRootDirPath, boolean covered) throws Exception {
			...
			for(File childFile : files) {
				if(childFile.isDirectory()) {
					...
					if(!skip) {
						...
						this.writeToFile(modelClazz, superControllerClazz, controllerPackageName, serviceClazz, author, outRootDirPath, covered);
					}
				}
			}
		}
	...
	public void writeToFile(Class<?> modelClazz, Class<?> superControllerClazz, String controllerPackageName, Class<?> serviceClazz, String author, String outRootDirPath, boolean covered) {
		String content = this.makeContent(modelClazz, superControllerClazz, controllerPackageName, serviceClazz, author);
	...
	public void writeToConsole(Class<?> modelClazz, Class<?> superControllerClazz, String controllerPackageName, Class<?> serviceClazz, String author) {
		String content = this.makeContent(modelClazz, superControllerClazz, controllerPackageName, serviceClazz, author);
		System.out.println(content);
	}
	
	protected String makeContent(Class<?> modelClazz, Class<?> superControllerClazz, String controllerPackageName, Class<?> serviceClazz, String author) {
		...
		if(null!=superControllerClazz) {
			if(!superControllerClazz.getPackage().getName().equals(controllerPackageName)) {
				content = content.replace("##import_base_controller_full_name##", "import " + superControllerClazz.getName() + ";");
			} else {
				content = content.replace("##import_base_controller_full_name##", "");
			}
			content = content.replace("##base_controller_name##", "extends " + superControllerClazz.getSimpleName() + " ");
		} else {
			content = content.replace("##import_base_controller_full_name##", "");
