...
<script type="text/javascript">
	...
	$(function() {
		$dg = $('#dg_id');
		$dg.datagrid({
		    url:'workflow/processDefinition/listPage',
		    width: 'auto',
	        ...
	        columns: [[
						...
						{field: 'diagramResourceName', title: 'PNG名称', width: parseInt($(this).width()*0.15), align: 'left',
							formatter: function(value, row) {
								return '<a href="javascript:void(0);" onClick="loadResource(\''+row.id+'\',\'image\');" width="950" style="color: blue">'+value+'</a>';
							}	
						},
						{field: 'suspendSatus', title: '是否挂起', width: parseInt($(this).width()*0.05), align: 'left',
							formatter: function(value, row) {
								if("1"==row.userStatus)
						...
						{field: 'deploymentTime', title: '流程发布时间', width: parseInt($(this).width()*0.1), align: 'left'},
						{field: 'operate', title: '操作', width: parseInt($(this).width()*0.1), align: 'center',
							formatter: function(value, row) {
								var accountCode = row.userCode;
								var a1 = '[<a href="javascript:void(0);" onClick="updateState(\''+row.id+'\',\'active\');" width="950" style="color: blue">激活</a>]';
								var a2 = '[<a href="javascript:void(0);" onclick="updateState(\''+row.id+'\',\'suspend\');" width="950" style="color: blue">挂起</a>]';
								var a3 = '[<a href="javascript:void(0);" onclick="changeStatus(\''+accountCode+'\',\'A\');" width="950" style="color: blue">转化为Model</a>]';
								return a1 + '&nbsp;' + a2 + '&nbsp;' + a3;
							}	
						}
	                   ]]
	...
	function openAddDlg() {
		parent.$.modalDialog({
			...
			width: 600,
			height: 400,
			href: 'workflow/app/leave/leaveEditDlg.jsp',
			buttons: [{
				text: '提交',
				iconCls: 'icon-ok',
				handler: function() {
					...
					editForm.attr("action", "workflow/app/leave/apply");
					editForm.submit();
				}
			},{
				text: '保存草稿',
				...
				handler: function() {
					...
					parent.$.modalDialog.openner = $dg;//定义对话框关闭要刷新的grid
					var editForm = parent.$.modalDialog.handler.find("#form_id");
					editForm.attr("action", "workflow/app/leave/add");
					editForm.submit();
				}
			},{
				text: '取消',
...
<body class="easyui-layout,fit:true">
	...
	<div data-options="region:'center',border:false" style="overflow:hidden;">
		<table id="dg_id" title="请假申请"></table>
	</div>
		...
		<table cellpadding="0" cellspacing="0">
			<tr>
				<td style="padding-left:2px;padding-bottom:2px;">
					...
					<a class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:'true'" onclick="openAddDlg();" href="javascript:void(0);">申请</a>
					</shiro:hasPermission>
