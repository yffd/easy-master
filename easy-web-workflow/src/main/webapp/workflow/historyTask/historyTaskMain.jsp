...
<script type="text/javascript">
	...
	$(function() {
		$dg = $('#dg_id');
		$dg.datagrid({
		    url:'workflow/historyTask/listPage',
		    width: 'auto',
...
// 	    	                 {field: 'workFlowDesc', title: '流程描述', width: 200, align: 'left'}  
	        ...
	        columns: [[
						...
						{field: 'assignee', title: '签收人ID', width: 200, align: 'left'},
						{field: 'claimTime', title: '签收时间', width: 150, align: 'center',
						...
						{field: 'operate', title: '操作', width: 300, align: 'center',
							formatter: function(value, row) {
								...
								var a2 = '[<a href="javascript:void(0);" onClick="findVariables(\''+row.instanceId+'\',\''+row.id+'\');" width="100" style="color: blue">查看变量</a>]';
	...
	function findTaskCandidators(taskId) {
		...
		parent.$.modalDialog({
			...
			href: 'workflow/historyTask/historyCandidatorsDlg.jsp',
			buttons: [{
				text: '关闭',
				iconCls: 'icon-cancel',
				handler: function() {
	...
	// 查看参数
	function findVariables(instanceId, taskId) {
		parent.$.modalDialog.instanceId=instanceId;
		parent.$.modalDialog.taskId=taskId;
		parent.$.modalDialog.width=950;
		parent.$.modalDialog({
			title: "任务参数信息",
			width: 950,
			height: $(this).height()-200,
			href: 'workflow/historyTask/historyVariablesDlg.jsp',
			buttons: [{
				text: '关闭',
				iconCls: 'icon-cancel',
				handler: function() {
