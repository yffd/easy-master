 ...
 * @see 	 
 */
public class CommonServiceSupport extends EasyModelConverter {
	...
	protected Long selectCountBy(String sqlId, Object parameter, boolean shortName) {
		if(EasyStringCheckUtils.isEmpty(sqlId)) throw EasyBizException.newInstance("sqlId 不能为空");
	...
	protected <T> List<T> selectRangeList(Map<String, Object> paramMap, PageParam pageParam, String sqlId, String countSqlId, boolean shortName) {
		...
		if(null!=pageParam) {
			if(EasyStringCheckUtils.isEmpty(countSqlId)) throw EasyBizException.newInstance("countSqlId 不能为空");
