...
<script type="text/javascript">
	...
	$(function() {
		...
		$dg.treegrid({
	        ...
	        columns: [[
						{field: 'orgCode', title: '编号', width: parseInt($(this).width()*0.1), align: 'left'},
	...
	function openAddDlg() {
		...
		parent.$.modalDialog({
			...
			width: 600,
			height: 400,
			href: 'jsp/auth/organization/organizationEditDlg.jsp',
			onLoad: function() {
				if(row){
			...
			buttons: [{
				...
				handler: function() {
					...
					var editForm = parent.$.modalDialog.handler.find("#form_id");
					editForm.attr("action", "auth/org/add");
					editForm.submit();
				}
			},{
				text: '取消',
			...
			parent.$.modalDialog({
				...
				buttons: [{
					...
					handler: function() {
						...
						parent.$.modalDialog.openner = $dg;//定义对话框关闭要刷新的grid
						var editForm = parent.$.modalDialog.handler.find("#form_id");
						editForm.attr("action", "auth/org/edit");
						editForm.submit();
					}
				},{
					text: '取消',
	...
	function removeFunc() {
		...
		if(row) {
			parent.$.messager.confirm("提示","确定要删除记录吗?",function(r){  
			    if(r) {
			    	$.post("auth/org/del", {orgCode: row.orgCode}, function(result) {
