...
// 			height: 28,
	...
	function openAddDlg() {
		parent.$.modalDialog({
			...
			width: 600,
			height: 400,
			href: 'jsp/pmi/user/userEditDlg.jsp',
			buttons: [{
				text: '确定',
				iconCls: 'icon-ok',
				handler: function() {
					...
					editForm.attr("action", "pmi/user/add");
					editForm.submit();
				}
			},{
				text: '取消',
			...
			parent.$.modalDialog({
				...
				buttons: [{
					...
					handler: function() {
						...
						parent.$.modalDialog.openner = $dg;//定义对话框关闭要刷新的grid
						var editForm = parent.$.modalDialog.handler.find("#form_id");
						editForm.attr("action", "pmi/user/edit");
						editForm.submit();
					}
				},{
					text: '取消',
	...
	function removeFunc() {
		...
		if(row) {
			parent.$.messager.confirm("提示","确定要删除记录吗?",function(r){  
			    if(r) {
			    	$.post("pmi/user/del", {userCode:row.userCode}, function(result) {
